...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float y = (DTid.y * cubeSize + offset) / extent.y;
    float z = (DTid.z * cubeSize + offset) / extent.z;
    ...
    float4x4 world = float4x4(1, 0, 0, 0,
        ...
        origin.z + DTid.z * cubeSize + offset,
        1.0f
        );
    VisualPackage v;
    v.world = world;
    v.normal = float4(1, 1, 1, 1);
    v.color = float4(color, .5f);

...
{
	m_hThread = (HANDLE)_beginthreadex(nullptr, 0, CReadChangesServer::ThreadStartProc,
...
void CReadDirectoryChanges::AddDictory(const std::wstring& szDictory, bool bWatchSubtree, DWORD dwNotifyFilter,
                                       DWORD dwBufferSize)
{
	if (!m_hThread)
		Init();
	CReadChangesRequest* pRequest = new CReadChangesRequest(m_pServer, szDictory, bWatchSubtree, dwNotifyFilter,
	                                                        dwBufferSize);

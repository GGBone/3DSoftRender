...
Material::Material(const shared_ptr<Renderer>& renderer)
	: m_Renderer(renderer),
	  m_Dirty(false)
{
	m_pProperties = std::shared_ptr<MaterialProperties>(
		static_cast<MaterialProperties*>(_aligned_malloc(sizeof(MaterialProperties), 16)));
...
Material::~Material()
{
	...
	{
		m_Renderer->destroy_constant_buffer(m_pConstantBuffer);
...
void Material::bind(std::shared_ptr<Shader> wpShader)
{
	...
	if (m_Dirty)
	{
		...
		shared_from_this()->UpdateConstantBuffer();
		shared_from_this()->m_Dirty = false;
	}
	for (auto texture : m_Textures)
	{
		//std::shared_ptr<TextureDX11>  pTexture = texture.second;
		texture.second->bind(static_cast<uint32_t>(texture.first), pShader->GetType());
	}

	ShaderParameter& materialParameter = pShader->GetShaderParameterByName("Material");
	if (materialParameter.is_valid())
	{
...
const Float4& Material::GetGlobalAmbientColor() const
{
	return m_pProperties->m_GlobalAmbient;
}

void Material::SetGlobalAmbientColor(const Float4& globalAmbient)
{
	m_pProperties->m_GlobalAmbient = globalAmbient;
	m_Dirty = true;
}

const Float4& Material::GetAmbientColor() const
{
	return m_pProperties->m_AmbientColor;
}

void Material::SetAmbientColor(const Float4& ambient)
{
	m_pProperties->m_AmbientColor = ambient;
	m_Dirty = true;
}

const Float4& Material::GetDiffuseColor() const
{
	return m_pProperties->m_DiffuseColor;
}

void Material::SetDiffuseColor(const Float4& diffuse)
{
	m_pProperties->m_DiffuseColor = diffuse;
	m_Dirty = true;
}

const Float4& Material::GetEmissiveColor() const
{
	return m_pProperties->m_EmissiveColor;
}

void Material::SetEmissiveColor(const Float4& emissive)
{
	m_pProperties->m_EmissiveColor = emissive;
	m_Dirty = true;
}

const Float4& Material::GetSpecularColor() const
{
	return m_pProperties->m_SpecularColor;
}

void Material::SetSpecularColor(const Float4& specular)
{
...
const Float4& Material::GetReflectance() const
{
	return m_pProperties->m_Reflectance;
}

void Material::SetReflectance(const Float4& reflectance)
{
...
bool Material::IsTransparent() const
{
	return (m_pProperties->m_Opacity < 1.0f ||
		...
		m_pProperties->m_AlphaThreshold <= 0.0f);
	// Objects with an alpha threshold > 0 should be drawn in the opaque pass.
}

...
StandardMesh::StandardMesh(VertexFormat* vformat, bool isStatic, bool inside, const Transform* transform)
	:
	mVFormat(vformat),
	mIsStatic(isStatic),
	mInside(inside),
...
TriMesh* StandardMesh::ExternalModel(int numOfAttribute, ...)
{
	if (numOfAttribute < 0)
		return nullptr;
	int numVertices = 0;
...
void StandardMesh::ReverseTriangleOrder(int numTriangle, int* indices)
{
}

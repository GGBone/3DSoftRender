...
namespace Hikari
{
	...
	public:
		...
		virtual void SetIndexBuffer(std::shared_ptr<Buffer> buffer)override;
		virtual void SetMaterial(std::shared_ptr<Material> buffer) override;
		virtual void SetInstanceBuffer(std::shared_ptr<Buffer> buffer)override;
	...
	private:
		...
		std::shared_ptr<Buffer> m_pInstanceBuffer;

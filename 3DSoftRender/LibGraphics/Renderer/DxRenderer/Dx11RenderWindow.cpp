...
static DXGI_RATIONAL QueryRefreshRate(UINT screenWidth, UINT screenHeight, BOOL vSync)
{
	...
	if (vSync)
	{
		...
		if (adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numDisplayModes,
		                                      nullptr))
		...
		if (FAILED(adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &
			numDisplayModes, displayModeList)))
		{
		}
...
Dx11RenderWindow::Dx11RenderWindow(WindowApplicationBase& app, HWND hwnd, std::shared_ptr<DirectRenderer>& device,
                                   const std::string& windowName, int iWindowWidth, int iWindowHeight, bool vSync)
...
void Dx11RenderWindow::ShowWindow()
{
	::ShowWindow(m_hWnd, SW_SHOWDEFAULT);
	BringWindowToTop(m_hWnd);
}

void Dx11RenderWindow::HideWindow()
{
	::ShowWindow(m_hWnd, SW_HIDE);
}

void Dx11RenderWindow::CloseWindows()
{
...
void Dx11RenderWindow::CreateSwapChain()
{
	...
	if (FAILED(factory->CreateSwapChainForHwnd(m_Device->GetDevice(), m_hWnd, &swapChainDesc, &swapChainFullScreenDesc,
		nullptr, &pSwapChain)))
	{
	}
...
void Dx11RenderWindow::OnPreRender(RenderEventArgs& e)
{
	if (m_bResizePending)
	{
...
void Dx11RenderWindow::OnRender(RenderEventArgs& e)
{
	base::OnRender(e);
}

void Dx11RenderWindow::OnPostRender(RenderEventArgs& e)
{
	base::OnPostRender(e);
}

void Dx11RenderWindow::OnResize(ResizeEventArgs& e)
{
	//Delay one frame
	base::OnResize(e);
	m_bResizePending = true;
...
void Dx11RenderWindow::OnTerminate(EventArgs& e)
{
	base::OnTerminate(e);
}

void Dx11RenderWindow::ResizeSwapChanBuffers(uint32_t width, uint32_t height)
{
	width = max(width, 1);
	height = max(height, 1);
...
void Dx11RenderWindow::Present()
{
	...
	std::shared_ptr<TextureDX11> colorBuffer = std::static_pointer_cast<TextureDX11>(
		m_RenderTarget->GetTexture(RenderTarget::AttachmentPoint::Color0));
	if (colorBuffer)
	{
...
std::shared_ptr<RenderTarget> Dx11RenderWindow::GetRenderTarget()
{
